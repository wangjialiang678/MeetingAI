...
## 2026-07-18 - 滚动替换缺陷修复：活跃 partial 按覆盖比例裁剪（用户现场反馈）

### 缺陷
- 用户在 20:13 场反馈"上面说话人区回填了，下面实时转写没有被替换"。用户对"不是阿里云问题"的判断正确——本次系统侧全健康（分析 5/5、覆盖追到 20:28），是 18:02 滚动替换实现的缺陷
- 根因：替换只隐藏"已冻结的 final 条目"，但实时流几乎全是一条持续累积的巨型 partial（timestamp 每次更新都刷新，永不早于覆盖游标），永远不会被隐藏；固定"尾部 600 字"截断又恰好显示的多是已覆盖内容

### 变更
- `TranscriptEntry` 新增 `firstTimestamp`（条目首次出现时间；partial 被替换更新时保留），带默认值兼容全部构造点
- 新增 `TranscriptDisplayTrimmer.visibleSuffix`：按时间比例估算 partial 中已覆盖前缀并裁掉（覆盖比例 ×0.85 保守系数，宁少裁不多裁；最少保留 60 字尾巴；再套 600 字上限）。仅显示层，落盘数据不动
- `TranscriptView.displayText` 接入裁剪器

### 验证
- RED→GREEN：`transcript_store_smoke` 新增 4 用例（半覆盖保守裁剪、无覆盖仅限长、全覆盖保留最小尾巴、短文本不动）→ PASS
- `swift build` + `bash tests/run-p0-p1.sh` → PASS；生效需重启 App（当前 19:40 实例含旧显示逻辑）
- 已知近似：partial 内无逐字时间戳，按时间线性估算，语速突变时裁剪点有偏差（保守系数兜底）；彻底方案需 ASR 送逐句 final（另列待办）
...
### 背景
- 用户口述需求：产品不只是会议记录，核心是"对话进行中的实时 AI 辅助"（销售拜访、教练会谈等）；手动提问是基本盘，AI 主动建议是加分项；要求从零复盘设计、产出下一版迭代设计文档
...
- ASR 骨干与级联架构保留不动；新增对话事件层（话轮边界 + 语义事件 + 全局热键三触发源）与快/深双车道推理（快车道流式首字 ≤3s）
