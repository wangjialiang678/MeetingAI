...
| A2 音频原生 | 实时多模态模型直接旁听音频流 | 理论延迟最低，能听到语气/情绪；但"持续旁听 + 默认沉默"不是这类 API 的设计姿态，成本与可控性存疑 |
...
| B2 话轮边界 | VAD/endpointing：每当一个人说完一段话 | 粒度贴近对话结构，天然低延迟锚点 |
...
| C3 三层 | 本地小模型初筛"值不值得说"→ 云端快车道 → 云端深车道；本地初筛还能省钱省隐私 |
...
麦克风 → AVAudioEngine → asr-bridge → DashScope 流式 ASR          【骨干不动】
   ...
   │    话轮边界（VAD/endpointing）+ 轻量语义扫描（先规则+词典，后小模型）
...
归档轨：Fun-ASR 双轨保留；并行启动 FluidAudio 中文 spike，
        达标后演进为本地近实时说话人轨（消除 OSS 依赖与分钟级回填延迟）
```
...
| D2 | 触发源从"字数/沉默阈值"换为三类：话轮边界、语义事件、热键 | G1；Balto"延迟 3 秒即侵蚀信任"；语义 VAD 已产品化 |
...
| D7 | 离线评估集先于语义触发器上线（M2 门槛） | G7；Calabrio"59% 从不刷新"、Sinch 回滚率警示 |
| D8 | 信任姿态延续：来源分级、参会者知情、不拿隐身当卖点 | openspec 已有方向 + Cluely 反例 |
| D9 | 快车道模型经小型 benchmark 选定（config.json `ai.fastModel` 可配），不绑供应商 | 沿用 `ai.apiKeyEnv` 的配置化传统；2026-03 已有后端 benchmark 方法可复用 |
...
> 原则：每个里程碑独立可用、独立可验证；先交付用户口述的基本盘（M1），主动性其后叠加。
...
**M2 语义事件触发（P1 的时机型提示）**
- 前置门槛：**离线评估集**——5-10 段销售/教练/会议模拟转写 + 人工标注"该说/不该说"（5-23 报告已建议，至此必须落地）
...
**M4 探索性 spike（并行、不阻塞主线）**
- FluidAudio 中文说话人分离基准测试（对照 Fun-ASR 轨；需带 ground truth 的中文会议录音）
- Gemini Flash Live"音频入/文字出"作 ASR 层候选的成本+质量联合验证（需申请 Gemini key）
- 跨会/客户维度记忆（销售客户历史、教练个案历史）——进入 openspec 单独提案；形态参考 Coach 的 Prep brief（会前简报）与 Follow-up（会后交付物），前置依赖 sessions 索引基础设施

落地方式：本文档评审通过后，转化为 openspec change（建议名 `realtime-copilot-v2`），与既有 `clarify-active-meeting-copilot`（speaking budget/研究队列/信任分级）合并演进，不冲突。
...
1. **首发场景选哪个？** 建议教练/私董会优先（竞品空白 + 你自己是重度用户，可自证价值），销售剧本第二批——但你更常接触哪类真实对话，应以实际可测试场景为准。
2. **悬浮条形态**：主窗口最小化时的极简悬浮条要不要进 M3？（涉及"参会者知情"姿态的呈现方式。注：对标 Coach 的"屏幕共享隐形悬浮层"已明确不抄——我们若做悬浮条，走可见、知情的姿态）
3. **macOS 最低版本策略**：维持 14+（放弃 SpeechAnalyzer）还是 `#available` 双路径？
4. **快车道模型偏好**：是否有希望优先测试的供应商（现有 OpenRouter 账号内选型最省事）？
5. 既有 openspec 开放问题继续有效（快查是否自动执行、调研任务是否跨会存活等）。
