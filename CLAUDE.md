...
| ASR 后端 | audio-asr-go（Go 子进程，:18080），WebSocket，DashScope 协议 |
...
│   ├── ASRClient.swift         # WebSocket 客户端，连接 asr-server
...
# 手动清理遗留的 asr-server 子进程（swift run 强制退出时可能遗留）
pkill asr-server
```
...
```json
{
  "asr": { "serverPort": 18080, "language": "zh" },
  "ai": {
...
```
麦克风 (AVAudioEngine) → AudioRecorder (PCM16) → ASRClient (WebSocket)
                              ↓ MP3                      ↓
                         本地文件              asr-server (Go 子进程)
                                                         ↓ DashScope ASR
...
Go 项目路径（硬编码在 `ASRServerManager.swift`）：
```
/Users/michael/projects/组件模块/audio-asr-suite/go/audio-asr-go
```

启动流程：检查 `bin/asr-server` 是否存在 → 不存在则调用 `go build` 编译 → 启动进程 → 健康检查（最多等 15 秒，每 500ms 轮询 `GET /healthz`）。
