...
│   ├── hotword.go              # 热词加载
│   ├── env.go                  # 环境变量加载（~/.Codex/api-vault.env）
...
# 运行（需要麦克风权限 + ~/.Codex/api-vault.env 中的 API Key）
swift run MeetingAI
...
**API 密钥**（从 `~/.Codex/api-vault.env` 自动读取，格式 `KEY=VALUE`）：
```
...
- 自动化基线：`bash tests/run-p0-p1.sh`
- 原生 GUI 自动化前置：macOS Accessibility 需对 `System Events` 开启；未开启时视为环境阻塞，不是代码通过

### 编码纪律

1. 每完成一个逻辑单元，至少重跑相关的 P0/P1 自动检查。
2. 任何涉及会议上下文、分析触发、ASR 集成的改动，都必须更新 `docs/test-plan.md` 或说明为何不需要更新。
3. 任何涉及原生 GUI 行为的改动，都要先跑 `tests/ui_accessibility_precheck.sh`，再决定能否执行自动 GUI 验证。
4. 若 GUI 自动化被系统权限阻塞，必须在 `docs/dev-log.md` 记录阻塞事实和影响范围。

### 失败处理

- 自动化 FAIL：先修代码或脚本，再重跑，不允许带着失败结果交付。
- 环境 BLOCKED：允许继续交付非 GUI 改动，但必须明确写出阻塞条件。
- 回归问题：记录到 `docs/dev-log.md`，包含测试项、现象、修复动作。
